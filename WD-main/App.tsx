        ...
        const checkSubscription = async () => {
            if ('Notification' in window && window.Notification.permission === 'granted') {
                try {
                    ...
                    const token = await firebase.messaging().getToken({ vapidKey: VAPID_PUBLIC_KEY });
                    if (token) {
        ...
        try {
            ...
            if (permission === 'granted') {
                ...
                if (token) {
                    ...
                    console.info('In a real application, this token would be sent to your application server to enable sending push notifications to this client.');
                    setIsSubscribed(true);
                } else {
                    console.warn('Could not get FCM token. Permission might be required or service worker not ready.');
                    setIsSubscribed(false);
                }
            } else {
